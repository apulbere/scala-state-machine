...
class SimpleSpec extends FlatSpec with BeforeAndAfter with Matchers{
  ...
  "the state machine" should "acquire S2 status" in {
    ...
    stateMachine.getState() should equal("S2")
  }

  "the state machine" should "acquire S3 status" in {
    stateMachine.sendEvent("E2")
    stateMachine.getState() should equal("S3")
  }
  ...
  "the state machine" should "preserve for an unknown event" in {
    ...
    stateMachine.getState() should equal("S1")
  }
}
